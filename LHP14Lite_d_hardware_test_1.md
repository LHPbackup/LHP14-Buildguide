...
**性能のばらつきが大きいジョイコンスティックでも使用可能になるかもしれません。(special thanks まるしぃ)**  

**・OLEDにXとYの現在値、最大値、最小値が表示されますので、ジョイスティックを左右上下にグリグリ動かしてX、Yそれぞれの最小値、スティック中立値、最大値をチェックしておきます。**
...
**注意：従来のProMicro（MCU=ATmega32U4）用のテストファームは何故か不安定で、LayerのOLED表示切替が遅延します。RP2040版は安定しており、推奨です。[秋月のRP2040 Kee Boar Driver](https://akizukidenshi.com/catalog/g/g117312/)は高速・高性能・低価格でおすすめ。**

**ATmega32U4をお使いの際は、Layerボタンを1回押し、1行1列目のキー（一番左上のキー）を押してLEDを消灯すると、若干遅延が解消するようです。Testファーム以外の通常使用時では遅延せず使えると思います。**
...
2025/8月現在最新のQMK 0.29.xは、ATmega32U4搭載のProMicroではアナログスティックの挙動が不安定になるようです。  
（数年前の古いQMKでは安定していますが、Joystick関連コマンドの遅延が大きい）
...
**ただし、QMK最新版とATmega32U4の組み合わせでFF14を遊んでみましたが、問題なく動作はするようです。**

**RP2040のADCにもハードウエアレベルのバグ（errata RP2040-E11）がありますが、QMKとLHP14の組み合わせでは安定動作しております。**
<br>
<br>
<br>
